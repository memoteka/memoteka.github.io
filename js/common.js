...
(function() {
  function createMobileBar() {
    ...
    const links = [
      { name: 'Главная', href: '/ru/', icon: '<svg viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>' },
      { name: 'Мемы', href: '/ru/memes/', icon: '<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M8 14s1.5 2 4 2 4-2 4-2"/><line x1="9" y1="9" x2="9.01" y2="9"/><line x1="15" y1="9" x2="15.01" y2="9"/></svg>' },
      { name: 'О нас', href: '/ru/about/', icon: '<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>' },
      { name: 'Донат', href: '/ru/donate/', icon: '<svg viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l8.72-8.72 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>' }
    ];

    const currentPath = window.location.pathname;

    bar.innerHTML = links.map(link => {
      // Простая логика: если путь совпадает, вешаем класс active
      const isActive = currentPath === link.href ? 'active' : '';
      return `
        <a href="${link.href}" class="mobile-tabbar-item ${isActive}">
          ${link.icon}
          <span>${link.name}</span>
        </a>
      `;
